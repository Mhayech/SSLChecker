...
try:
    ...
    print('Please install required modules: pip install -r oussama_requires.txt')
    sys.exit(1)
...
class SSLChecker:
    ...
    def analyze_ssl(self, host, context, user_args):
        #Analyze the security of the SSL cert.
        ...
        while True:
            ...
            request = json.loads(urlopen(api_url + 'analyze?host={}'.format(host)).read().decode('utf-8'))
            if request['status'] in ('DNS', 'IN_PROGRESS'):
                if user_args.verbose:
        ...
        endpoint_data = json.loads(urlopen(api_url + 'getEndpointData?host={}&s={}'.format(
            host, request['endpoints'][0]['ipAddress'])).read().decode('utf-8'))
    ...
    def get_cert_sans(self, x509cert):
        # SAN (Subject Alt Name is used when the certificate is used for more than one website) 
        # Get all the hostnames that use the same cert
        san = ''
    ...
    def get_cert_info(self, host, cert):
        ...
        # If validity of cert is less thqn 15
    ...
    def print_status(self, host, context, analyze=False):
        # Import https_module
        from https_checker import https_checker
        """Print all the usefull info about host."""
        print('\t{}[+]{} {}\n\t{}'.format(Clr.GREEN, Clr.RST, host, '-' * (len(host) + 5)))
        print("\t\tSupport Https :{}".format(https_checker(host)))
    ...
    def show_result(self, user_args):
        ...
        if not user_args.json_true and user_args.analyze:
            print('{}Warning: -a/--analyze is enabled. It takes more time, Be patient...{}\n'.format(Clr.YELLOW, Clr.RST))
        ...
        for host in hosts:
            ...
            try:
                ...
                # Analyze the certificate if -a/--analyze
        ...
        if user_args.json_save_true:
            for host in context.keys():
                with open(host + '.json', 'w', encoding='UTF-8') as file:
                    file.write(json.dumps(context[host]))
    ...
    def get_args(self, json_args={}):
        ...
        parser.add_argument("-https","--https-check",dest="status",default=True,action="store_true",help="Check HTTPS")
